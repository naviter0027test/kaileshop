...
class SubscriptionController extends BaseController
{
    ...
    public function confirmNewebPay($package_id, Request $request)
    {
        if (!auth()->user()->can('subscribe')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            ...
            //Add subscription details after payment is succesful
            $subscription = $this->_add_subscription($business_id, $package, request()->gateway, $payment_transaction_id, $user_id);
            DB::commit();

            $msg = __('lang_v1.success');
            if (request()->gateway == 'neweb') {
                $msg = __('superadmin::lang.notification_sent_for_approval');
            }
            $output = ['success' => 1, 'msg' => $msg];
        } catch (\Exception $e) {
